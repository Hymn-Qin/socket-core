...
/**
 * {@link DecodingState} which consumes all socketCallback bytes until the session is
 * closed.
 *
...
public abstract class ConsumeToEndOfSessionDecodingState implements DecodingState {
    ...
    public DecodingState decode(IoBuffer in, ProtocolDecoderOutput out) throws Exception {
        ...
        if (buffer.position() + in.remaining() > maxLength) {
            throw new ProtocolDecoderException("SocketCallback data exceeds " + maxLength + " byte(s).");
        }
        buffer.put(in);
        return this;
    }
