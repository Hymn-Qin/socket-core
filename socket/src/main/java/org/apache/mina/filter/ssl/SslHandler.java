...
class SslHandler {
    ...
    /**
     * Push the newly socketCallback data into a queue, waiting for the SSL session
    ...
    /* no qualifier */void messageReceived(NextFilter nextFilter, ByteBuffer buf) throws SSLException {
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("{} Processing the socketCallback message", sslFilter.getSessionInfo(session));
        }
        ...
        switch (status) {
            case BUFFER_OVERFLOW:
                throw new SSLException("SSLEngine error during decrypt: " + status + " inNetBuffer: " + inNetBuffer
                    + "appBuffer: " + appBuffer);
            case CLOSED:
                Exception exception =new RuntimeIoException("SSL/TLS close_notify socketCallback");
